...
public class Vala.GIRWriter : CodeVisitor {
	...
	public override void visit_class (Class cl) {
		...
		if (!cl.is_compact) {
			...
			if (cl.base_class == null) {
				...
				buffer.append_printf (" glib:ref-func=\"%s\"", CCodeBaseModule.get_ccode_ref_function (cl));
				buffer.append_printf (" glib:unref-func=\"%s\"", CCodeBaseModule.get_ccode_unref_function (cl));
				buffer.append_printf (" glib:set-value-func=\"%s\"", CCodeBaseModule.get_ccode_set_value_function (cl));
				buffer.append_printf (" glib:get-value-func=\"%s\"", CCodeBaseModule.get_ccode_get_value_function (cl));
			} else {
			...
			write_indent ();
			if (cl.base_class == null) {
				buffer.append_printf ("<type name=\"%s\" c:type=\"%s\"/>\n", gi_type_name (gtypeinstance_type), CCodeBaseModule.get_ccode_name (gtypeinstance_type));
			} else {
	...
	private void write_params_and_return (List<Parameter> params, List<TypeParameter>? type_params, DataType? return_type, bool return_array_length, string? return_comment = null, bool constructor = false, DataType? instance_type = null, bool user_data = false) {
		...
		if (params.size != 0 || (type_params != null && type_params.size > 0) || instance_type != null || (return_type is ArrayType && return_array_length) || (return_type is DelegateType) || ret_is_struct) {
			...
			foreach (Parameter param in params) {
				write_param_or_return (param.variable_type, true, ref index, CCodeBaseModule.get_ccode_array_length (param), param.name, get_parameter_comment (param), param.direction, false, false, param.ellipsis);
	...
	public override void visit_delegate (Delegate cb) {
		...
		write_params_and_return (cb.get_parameters (), cb.get_type_parameters (), cb.return_type, CCodeBaseModule.get_ccode_array_length (cb), get_delegate_return_comment (cb), false, null, cb.has_target);
	...
	private void do_write_signature (Method m, string tag_name, bool instance, string name, string cname, List<Vala.Parameter> params, DataType return_type, bool can_fail, bool write_comment) {
		...
		write_params_and_return (params, m.get_type_parameters (), return_type, CCodeBaseModule.get_ccode_array_length (m), return_comment, false, instance_type);
	...
	private void write_type (DataType type, int index = -1, ParameterDirection direction = ParameterDirection.IN) {
		if (type is ArrayType) {
			...
			buffer.append_printf (" c:type=\"%s%s\"", CCodeBaseModule.get_ccode_name (array_type.element_type), direction == ParameterDirection.IN ? "*" : "**");
			buffer.append_printf (">\n");
			indent++;
			...
			write_indent ();
			buffer.append_printf ("<type name=\"gpointer\" c:type=\"%s%s\"/>\n", CCodeBaseModule.get_ccode_name (type), direction == ParameterDirection.IN ? "" : "*");
			...
			var deleg_type = (DelegateType) type;
			write_indent ();
			buffer.append_printf ("<type name=\"%s\" c:type=\"%s%s\"/>\n", gi_type_name (deleg_type.delegate_symbol), CCodeBaseModule.get_ccode_name (type), direction == ParameterDirection.IN ? "" : "*");
