	...
	public override void visit_cast_expression (CastExpression expr) {
		...
		if (expr.is_silent_cast) {
			if (is_basic_type && basic_type.is_string) {
				...
				if (target_type.is_real_non_null_struct_type ()) {
					ccode.add_assignment (new CCodeIdentifier ("*result"), get_cvalue_ (transform_value (temp_value, expr.target_type, expr)));
				} else {
			...
			if (target_type.is_real_non_null_struct_type ()) {
				ccode.add_assignment (new CCodeIdentifier ("*result"), new CCodeConstant ("NULL"));
			} else {
