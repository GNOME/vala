	...
	public override void visit_struct (Struct st) {
		...
		if (get_ccode_has_type_id (st)) {
			if (get_ccode_name (st).length < 3) {
				st.error = true;
				Report.error (st.source_reference, "Struct name `%s' is too short".printf (get_ccode_name (st)));
				return;
			}
	...
	public override void visit_enum (Enum en) {
		...
		if (get_ccode_has_type_id (en)) {
			if (get_ccode_name (en).length < 3) {
				en.error = true;
				Report.error (en.source_reference, "Enum name `%s' is too short".printf (get_ccode_name (en)));
				return;
			}
