...
```sh
curl --silent --show-error --location https://download.gnome.org/sources/vala/0.36/vala-0.36.16.tar.xz --output vala-bootstrap.tar.xz
...
If you wish to build a specific release, for example 0.36.16:

```sh
git checkout 0.36.16
git clean -dfx
./autogen.sh
make && sudo make install
```
